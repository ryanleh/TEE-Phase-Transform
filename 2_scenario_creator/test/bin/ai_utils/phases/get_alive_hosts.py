...
class GetAliveHostsPhaseClass(AbstractPhaseClass):
  TrackerId = "518"
  Subject = "Get Alive Hosts"
  Description = "This phase tries to identify what machines are up from the list of IPs"

  def __init__(self, is_phase_critical, ip_list=None, n_threads=50, timeout=1, use_arp=False):
    logging.debug('Executing __init__. is_phase_critical: {}, ip_list: {}, n_threads: {}, timeout: {}, use_arp: {}'.format(is_phase_critical, ip_list, n_threads, timeout, use_arp))
  ...
  def execute_scan(self):
    ...
    self.alive_hosts = NetworkScanUtilsClass.GetAliveHosts(self.ip_list, self.n_threads, self.timeout, useARP=self.use_arp)
  ...
  def log_success(self, phase_successful):
    ...
    if phase_successful:
      ...
      self.PhaseReporter.Report('{} alive hosts were found after scanning the network: {}'.format(len(self.alive_hosts), ', '.join(self.alive_hosts)))
      self.PhaseReporter.Mitigation('Traffic from "{}" to the following hosts should be monitored or prevented: {}'.format(HostInfo.GetLocalIpAddress(), ', '.join(self.alive_hosts)))
    else:
  ...
  def setup_ip_list(ip_list):
    ...
    if ip_list:
      ...
      param = NetworkUtils.GetLocalNetworkHosts()
